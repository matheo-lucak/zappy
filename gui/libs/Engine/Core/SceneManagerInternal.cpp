...
{
    (void)path;
    INDIE_CORE_ASSERT(m_paths.size(), "A build scene conf can be set just once");
    m_paths = deserializer::sceneConf::deserialize(path);
}

void SceneManagerInternal::AskLoadScene(unsigned int buildIdx)
{
    INDIE_CORE_ASSERT(buildIdx >= m_paths.size(), "Scene index out of range");
...
{
    INDIE_CORE_ASSERT(m_scene == nullptr, "A scene must be set");
...
{
    INDIE_CORE_ASSERT(m_scene == nullptr, "A scene must be set");
    DeepFindObject([&object](const auto &obj) { return obj == object; },
                    [](auto &list, auto i) {
...
ecs::GameObject *SceneManagerInternal::FindGameObjectByNameInList(const std::string &name, std::vector<std::unique_ptr<ecs::GameObject>> &list)
{
    ...
    DeepFindObjectHelper([&name](const auto &obj) { return obj.getName() == name; },
...
ecs::GameObject &SceneManagerInternal::FindPrefabByName(const std::string &name)
{
    ...
    INDIE_CORE_ASSERT(it == m_scene->prefabs.end(), "No prefab named '" + name + "'");
    return **it;
}
...
std::list<ecs::GameObject *> SceneManagerInternal::FindGameObjectsByTag(const std::string &tag)
{
    ...
    DeepFindObject([&tag](const auto &obj) { return obj.getTag() == tag; },
                    [&res](auto &list, auto i) {
...
{
    if (object.getActive() == false) return;
...
void SceneManagerInternal::OnComponentRemoved<ecs::BoxCollider>(ecs::GameObject &object)
{
    ...
    INDIE_CORE_ASSERT(it == m_scene->colliders.end(), "The corresponding object could not been found");
...
void SceneManagerInternal::OnComponentRemoved<ecs::Button>(ecs::GameObject &object)
{
    ...
    INDIE_CORE_ASSERT(it == m_scene->buttons.end(), "The corresponding object could not been found");
...
void SceneManagerInternal::OnComponentRemoved<ecs::MusicPlayer>(ecs::GameObject &object)
{
    ...
    INDIE_CORE_ASSERT(it == m_scene->musics.end(), "The corresponding object could not been found");
...
void SceneManagerInternal::OnComponentRemoved<ecs::Animator>(ecs::GameObject &object)
{
    ...
    INDIE_CORE_ASSERT(it == m_scene->animators.end(), "The corresponding object could not been found");
...
void SceneManagerInternal::OnComponentRemoved<Graphics::IDrawable2D>(ecs::GameObject &object)
{
    ...
    INDIE_CORE_ASSERT(it == m_scene->drawable2d.end(), "The corresponding object could not been found");
...
void SceneManagerInternal::OnComponentRemoved<Graphics::IDrawable3D>(ecs::GameObject &object)
{
    ...
    INDIE_CORE_ASSERT(it == m_scene->drawable3d.end(), "The corresponding object could not been found");
